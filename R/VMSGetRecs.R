...
#' limit how many VMS records are pulled on each function call.  It is in place 
...
VMSGetRecs <- function(usepkg = 'roracle', dateStart = NULL, dateEnd = NULL, 
                       hrBuffer = 4,  shp = NULL, shp.field=NULL, 
                       rowNum = 50000){
  whereDateEnd = paste0("AND ROWNUM <= ",rowNum)
  if (!is.null(dateEnd)) whereDateEnd = paste0("AND POSITION_UTC_DATE < to_date('",dateEnd,"','YYYY-MM-DD HH24:MI:SS')") 
  ...
  assign("oracle_cxn", make_oracle_cxn(usepkg), envir = .GlobalEnv )
